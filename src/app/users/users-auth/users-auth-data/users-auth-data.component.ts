...
import { UsersAuthService } from './services/users-auth-data.service';
@Component({
  selector: 'users-auth-data',
  templateUrl: './users-auth-data.component.html',
  styleUrls: ['./users-auth-data.component.css']
})
...
public getAuthUserList() {
    this.usersAuthService.getUsers()
      .subscribe(
        res => {
          var data = res.json();
          this.authUserList = data.auth_members;
  ...
  //分页，页码导航,要求参数为一个对象
public createPageNav(opt) {
    opt= opt || {};
    let $container   = opt.$container || null, //必需，页码容器，请确保这个容器只用来存放页码导航
